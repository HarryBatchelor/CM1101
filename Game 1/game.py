...
def normalise_input(user_input):
    ...
    'help'
    """
    s = remove_spaces(remove_punct(user_input)).lower()
    return s
...
def print_menu(exits):
    ...
    """
    print("You can:")
    n = exit_leads_to(exits,'north')
    e = exit_leads_to(exits,'east')
    w = exit_leads_to(exits,'west')
    s = exit_leads_to(exits,'south')
    if n !=None:
        print_menu_line('north',n)
    if e !=None:
        print_menu_line('east',e)
    if s !=None:
        print_menu_line('south',s)
    if w !=None:
        print_menu_line('west',w)
...
def menu(exits):
    ...
    while True:
        ...
        print_menu(exits)
        # Read player's input
        playerInput = normalise_input(str(input('> ')))
        # Normalise the input
